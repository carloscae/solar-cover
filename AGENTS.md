...
*   External cover moves (physical remote, other automations) are detected automatically by the coordinator via state-change listeners on the zone's physical cover entities. Any position change exceeding hysteresis and outside the 30-second post-command debounce window sets a manual override. Never call `_command_covers` directly from outside the coordinator.
