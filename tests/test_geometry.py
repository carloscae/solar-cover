...
class TestVerticalPosition:
    def test_midday_summer(self) -> None:
        ...
        # blind_height = (1.0 / cos(0)) * tan(60 deg) = 1 * 1.732 = 1.732m -> 1.732/2.5*100 ~= 69.3%
    ...
    def test_gamma_near_90_clips_to_0(self) -> None:
        # gamma=89 deg -- sun nearly at edge of FOV, cos(89 deg) is very small -> clip to h_win -> 100%
...
class TestHorizontalPosition:
    def test_midday_summer(self) -> None:
        # Sun at 60 deg, gamma=0, attach_height=2.5m, awn_length=3m, awn_angle=15 deg, distance=2m
...
class TestTiltPosition:
    ...
    def test_bidirectional_range(self) -> None:
        # Bidirectional should produce exactly half the single-direction % for same geometry.
        # sol_elev=30 gives single ~97% (not clamped), so the half-range relationship holds.
        single = tilt_position(
            sol_elev_deg=30.0, gamma_deg=0.0,
            slat_width_mm=80.0, slat_spacing_mm=50.0, bidirectional=False,
        )
        bidi = tilt_position(
            sol_elev_deg=30.0, gamma_deg=0.0,
    ...
    def test_negative_discriminant_returns_100(self) -> None:
        ...
        # With spacing=100mm and width=50mm (ratio=2), discriminant is strongly negative
        # at any realistic elevation -> formula returns 100% (fully closed).
        result = tilt_position(
            sol_elev_deg=2.0, gamma_deg=0.0,
            slat_width_mm=50.0, slat_spacing_mm=100.0, bidirectional=False,
        )
        assert result == 100.0
